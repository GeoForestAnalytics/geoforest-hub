    ...
    const unsubscribeAuth = auth.onAuthStateChanged(async (user) => {
      if (user) {
        ...
        try {
          ...
          // 1. Busca Atividades
          onSnapshot(query(collection(db, `clientes/${uid}/atividades`), where("projetoId", "in", [projId, Number(projId)])), (snap) => {
          ...
          // 4. Busca Amostras (dados_coleta)
          onSnapshot(query(collection(db, `clientes/${uid}/dados_coleta`), where("projetoId", "in", [projId, Number(projId)])), (snap) => {
            setAmostras(snap.docs.map(d => ({ id: d.id, ...d.data() })));
          });

          // 5. Busca Cubagens (dados_cubagem)
          onSnapshot(query(collection(db, `clientes/${uid}/dados_cubagem`)), (snap) => {
    ...
    <div className="p-8 max-w-7xl mx-auto bg-slate-50 min-h-screen">
      ...
      <div className="bg-white rounded-[32px] p-8 border border-slate-200 shadow-sm mb-8 flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium italic">Gestão de Inventário e Cubagem</p>
        </div>
        ...
        <div className="flex gap-3">
          <Link href={`/projetos/${projId}/analise`} className="flex items-center gap-2 bg-emerald-600 text-white px-6 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-900/20">
            <BarChart3 size={18} /> Consolidado de Campo
          </Link>
          <Link href={`/projetos/${projId}/cubagem`} className="flex items-center gap-2 bg-slate-900 text-white px-6 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-800 transition-all shadow-lg">
            <Ruler size={18} className="text-emerald-400" /> Cubagem do Projeto
          </Link>
        </div>
      </div>
      ...
      <div className="space-y-10">
        {atividades.map((ativ) => {
          // Lógica para identificar se é atividade de Cubagem
          ...
          return (
            <div key={ativ.id} className="bg-white rounded-[32px] border border-slate-200 overflow-hidden shadow-sm">
              <div className="bg-slate-900 p-5 px-8 flex justify-between items-center">
                ...
                <span className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">{ativ.metodoCubagem || "Padrão"}</span>
              </div>
                ...
                {fazendas.filter(f => f.atividadeId === ativ.id).map(faz => {
                  ...
                  return (
                    <div key={faz.id} className="space-y-4">
                      <div onClick={() => toggleFazenda(faz.id)} className="flex items-center justify-between cursor-pointer group bg-slate-50/50 p-3 rounded-2xl border border-transparent hover:border-emerald-500/20 transition-all">
                      ...
                      {!isMinimizada && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pl-4">
                          {talhoes.filter(t => t.fazendaId === faz.id).map(tal => {
                            ...
                            // LÓGICA DINÂMICA DE DADOS E LINKS
                            let total = 0;
                            let concluidas = 0;
                            let label = "Amostras";
                            let Icon = ListChecks;
                            ...
                            if (isCUB) {
                                // Se for cubagem, muda a URL, o rótulo e a fonte de dados
                                ...
                                // Se for inventário, usa amostras
