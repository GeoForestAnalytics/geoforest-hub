...
export default function FrotaPage() {
  ...
  const [termoBusca, setTermoBusca] = useState(""); // ✅ Estado para o filtro
  ...
  const dadosFiltrados = useMemo(() => {
    // 1. Processa o KM atual vindo do campo
    const processados = veiculos.map(v => {
      const registrosDoCarro = diarios.filter(d => normalizarPlaca(d.veiculoPlaca || "") === v.id);
    ...
    // 2. Aplica a busca (Placa, Modelo ou Motorista)
    ...
    // 3. Calcula KPIs baseados APENAS nos itens filtrados
    const kpis = {
  ...
  const handleSalvarVeiculo = async () => {
    ...
    if (veiculoEmEdicao) {
      ...
      await setDoc(doc(db, `clientes/${licenseId}/frota`, placaLimpa), { ...payload, kmAtual: Number(formData.kmInicial), status: 'operacional', dataCadastro: serverTimestamp(), custoManutencaoAcumulado: 0 });
    }
    setIsModalOpen(false);
  };

  const handleLancarManutencao = async () => {
    if (!veiculoSelecionado || !gastoForm.descricao || !gastoForm.valor) return;
  ...
  if (authLoading || loading) return <div className="p-20 text-center animate-pulse font-black text-slate-400 uppercase">Sincronizando Frota...</div>;
    ...
    <div className="p-8 max-w-7xl mx-auto bg-slate-50 min-h-screen font-sans">
      ...
      <header className="mb-10 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase">Gestão de Frota</h1>
        </div>
      ...
      {/* ✅ KPIs DINÂMICOS (Recalculam conforme o filtro) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm flex flex-col justify-between overflow-hidden relative">
            <Coins className="absolute -right-4 -bottom-4 text-emerald-500/10" size={120} />
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Manutenção (Filtrada)</p>
            <h2 className="text-3xl font-black text-slate-900">R$ {dadosFiltrados.kpis.custoTotal.toLocaleString('pt-BR')}</h2>
            <p className="text-[8px] text-slate-400 font-bold uppercase mt-2">Investimento nos {dadosFiltrados.kpis.frotaExibida} veículos exibidos</p>
        </div>
        <div className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm flex flex-col justify-between">
            ...
            <div>
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">KM Rodados (Seleção)</p>
        ...
        <div className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-sm flex flex-col justify-between">
            ...
            <div>
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Alertas na Seleção</p>
          ...
          <div key={v.id} className="bg-white rounded-[40px] p-8 border border-slate-200 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden flex flex-col">
            ...
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">{v.modelo}</p>
              </div>
              <button 
            ...
            <div className="space-y-3 mb-8">
                <div className="flex justify-between text-[9px] font-black uppercase tracking-widest text-slate-900">
                    <span className="text-slate-400">KM: {v.kmAtual.toLocaleString()}</span>
            ...
            >
              <History size={16} className="text-emerald-400" /> Histórico & Gastos
            </button>
          </div>
        ))}
      </div>

      {/* MODAL HISTÓRICO (PAINEL LATERAL) */}
          ...
          <div className="bg-white h-full w-full max-w-2xl shadow-2xl flex flex-col animate-in slide-in-from-right duration-500">
            <div className="p-10 bg-slate-50 border-b flex justify-between items-center text-slate-900">
              <div>
                ...
                <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">Oficina e Manutenção</p>
              </div>
            ...
            <div className="p-10 border-b bg-emerald-50/30 space-y-4 text-slate-900">
               <h3 className="text-xs font-black text-emerald-700 uppercase tracking-widest flex items-center gap-2"><Plus size={14}/> Lançar Manutenção</h3>
               ...
               <button onClick={handleLancarManutencao} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-700 transition-all">Registrar Pagamento</button>
            </div>
                ...
                {historico.map(h => (
                  <div key={h.id} className="flex justify-between items-center p-6 bg-slate-50 rounded-[32px] group border border-transparent hover:border-emerald-200 transition-all">
                    <div className="flex items-center gap-4">
                        <div className="bg-white p-3 rounded-2xl shadow-sm text-slate-400 group-hover:text-emerald-500"><Receipt size={20}/></div>
                        <div>
            ...
            <div className="p-10 bg-slate-900 text-white flex justify-between items-center shrink-0">
                <p className="text-xs font-black uppercase text-slate-500">Total Investido no Veículo</p>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[48px] shadow-2xl w-full max-w-md overflow-hidden border border-emerald-500/20 text-slate-900">
            <div className="p-10 bg-slate-50 border-b flex justify-between items-center">
              <h2 className="text-3xl font-black uppercase tracking-tighter">{veiculoEmEdicao ? "Editar Ativo" : "Novo Ativo"}</h2>
            ...
            <div className="p-10 space-y-4">
                ...
                <input placeholder="Modelo" className="w-full p-4 rounded-2xl bg-slate-50 font-bold outline-none" value={formData.modelo} onChange={e => setFormData({...formData, modelo: e.target.value})} />
                <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Motorista</label>
                <input placeholder="Motorista" className="w-full p-4 rounded-2xl bg-slate-50 font-bold outline-none" value={formData.motorista} onChange={e => setFormData({...formData, motorista: e.target.value})} />
